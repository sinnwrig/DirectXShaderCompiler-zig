...
pub fn build(b: *Build) !void {
    ...
    if (build_shared) {
        dxcompiler.defineCMacro("DXC_C_SHARED_LIBRARY", "");
        dxcompiler.defineCMacro("DXC_C_IMPLEMENTATION", "");
    }

    linkDxcDependencies(dxcompiler);
    dxcompiler.addIncludePath(b.path("src"));
    ...
    var opt_lib : *std.Build.Step.Compile = undefined;
    var val_lib : *std.Build.Step.Compile = undefined;
    ...
    if (build_spirv)
    {
        ...
        if (b.lazyDependency("SPIRV-Tools", .{
            .target = target,
            .optimize = optimize,
            .header_path = b.path("external/SPIRV-Headers").getPath(b),
            .no_link = true,
            .no_reduce = true,
        })) |dep| 
        {
            opt_lib = dep.artifact("SPIRV-Tools-opt");
            val_lib = dep.artifact("SPIRV-Tools-val");
        }

        dxcompiler.linkLibrary(opt_lib);
        dxcompiler.linkLibrary(val_lib);
    }
    ...
    dxc_exe.addIncludePath(b.path("include"));
    ...
    if (build_spirv)
    {
        dxc_exe.linkLibrary(opt_lib);
        dxc_exe.linkLibrary(val_lib);
    }
...
fn addIncludes(b: *Build, step: *std.Build.Step.Compile) void {
    ...
    const target = step.rootModuleTarget();
    if (target.os.tag != .windows) step.addIncludePath(b.path("external/DirectX-Headers/include/wsl/stubs"));
}
